...
FALLBACK_PATH = BASE_DIR / "ward_fallbacks.csv"
...
fallbacks = pd.read_csv(FALLBACK_PATH)

WARD_ID_TO_PROXY = dict(
    zip(
        fallbacks["ward_id"].astype(int),
        fallbacks["proxy_ward_name"].astype(str),
    )
)
...
def get_forecast(
    ...
    source_ward_name = WARD_ID_TO_PROXY.get(
    ward_id,
    ward_name,
)
    latest_24 = latest_contiguous_24_hours(source_ward_name)
